...
export default function CaseDetailPage() {
  ...
  const handleSend = async (patientType) => {
    try {
      await api.assignPatientType(patient.operationNo, patientType)
      setPatient((current) => ({ ...current, patientType: patientType.toLowerCase() }))
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/45" onMouseDown={onClose}><aside className="absolute inset-y-0 right-0 w-full max-w-[708px] overflow-y-auto bg-white shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-end gap-3 border-b border-slate-200 pb-5">
        ...
        <button onClick={() => onSend(patientType)} className="btn-primary inline-flex h-[38px] items-center gap-2 px-6"><Send size={14} />ส่งเคส</button>
      </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4">
      <section className="relative w-full max-w-[480px] rounded-2xl bg-white px-8 pb-7 pt-8 shadow-2xl">
        ...
        <div className="space-y-3 text-[14px]">
          {[['วันที่สร้าง', '15 มิ.ย. 2569 10:45 น.'], ['ผู้สร้าง', 'AdminHospitalBK (OR Nurse)'], ['ปลายทาง', patient.patientType?.toUpperCase() || '-'], ['แผนก', patient.department || '-']].map(([k, v]) => (

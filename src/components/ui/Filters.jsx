...
function ValidationFilters({ search, onSearch }) {
  ...
  return <section className="rounded-xl border border-black/10 bg-white p-[25px] shadow-sm">
    <h2 className="flex items-center gap-2 text-[17px] leading-6 font-medium text-[#191c1e]"><Search size={18} className="text-[#175beb]" />ค้นหาข้อมูล</h2>
    <div className="mt-4 grid gap-[10px] sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{fields.map((label, i) => <label key={`${label}-${i}`} className="flex flex-col gap-1 text-[14px] leading-[18px] text-[#424752]">{label}<select className="h-[39px] rounded-lg border border-[#e2e8f0] bg-white px-[13px] text-[14px] text-[#191c1e]"><option>ทั้งหมด</option></select></label>)}</div>
    <div className="mt-4 grid gap-[10px] lg:grid-cols-[201px_minmax(0,1fr)] xl:grid-cols-[201px_minmax(0,1fr)_246px]">
      <label className="flex flex-col gap-1 text-[14px] leading-[18px] text-[#424752]">ความเสี่ยง SSI<select className="h-[39px] rounded-lg border border-[#e2e8f0] bg-white px-[14px] text-[14px]"><option>ทั้งหมด</option></select></label>
      <label className="flex flex-col gap-1 text-[14px] leading-[18px] text-[#424752]">คำค้นหา<input value={search} onChange={(e) => onSearch(e.target.value)} className="h-[39px] rounded-lg border border-[#e2e8f0] px-[13px] text-[14px] outline-none" placeholder="ค้นหา HN, ชื่อผู้ป่วย, หัตถการ..." /></label>
      <div className="flex flex-wrap items-end gap-2 lg:col-span-2 xl:col-span-1"><button className="flex h-[39px] flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-[#175beb] px-3 text-[14px] font-medium tracking-[.6px] text-white"><Search size={14} />ค้นหา</button><button onClick={() => onSearch('')} className="flex h-[39px] flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-[#e2e8f0] px-[17px] text-[14px] font-medium tracking-[.6px] text-[#424752]"><RefreshCw size={13} />ล้างตัวกรอง</button></div>
    </div>
  </section>
}

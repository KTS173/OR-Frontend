...
function MyFollowUpsPage({ patients, loading, search, setSearch }) {
  return <div className="space-y-4"><section className="grid grid-cols-5 gap-4">{myMetrics.map(([label,value,unit,Icon,color],index)=><article key={label} className="flex h-[140px] flex-col rounded-xl border border-black/10 bg-white p-[17px] shadow-sm"><div className="flex justify-between"><p className="text-[14px] font-medium" style={{color}}>{label}</p><span className="grid size-9 place-items-center rounded-lg bg-slate-50" style={{color}}><Icon size={20}/></span></div><p className="mt-3 flex items-baseline gap-1.5 text-[28px] font-semibold leading-8" style={{color}}><span>{value}</span><span className="text-[14px]">{unit}</span></p><p className="mt-auto text-[12px] text-slate-500">{index===2?'คนไข้ไม่รับโทรศัพท์':index===3?'ติดตามเสร็จสิ้น':index===4?'เกินกำหนดแล้ว':index===5?'↑ 2 ราย จากเมื่อวาน':'ต้องติดตามวันนี้'}</p></article>)}</section><MyFilters search={search} setSearch={setSearch}/><section className="flex h-[66px] items-center rounded-xl border border-black/10 bg-white px-4 shadow-sm"><div className="flex h-full items-center gap-7">{['ทั้งหมด (52)','รายการกำหนดติดตามวันนี้ (48)','รายการยังไม่ถึงรอบติดตาม','รายการสงสัย SSI','รายการติดเชื้อ SSI'].map((x,i)=><button key={x} className={`h-full text-[13px] font-medium ${i===0?'border-b-2 border-[#175beb] text-[#175beb]':'text-[#424752]'}`}>{x}</button>)}</div></section><MyFollowTable patients={patients} loading={loading}/></div>
}

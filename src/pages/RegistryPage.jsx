import { AlertCircle, AlertTriangle, ArrowRight, CalendarClock, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, ClipboardCheck, Download, History, PhoneCall, Plus, RefreshCw, Search, Send, ShieldCheck, ShieldX, TimerReset, UserRound, X } from 'lucide-react'
...
const opdMetrics = [
  ['รอรับเคสใหม่', '18', 'คน', History, '#1e3a8a', '#eff6ff'],
  ['ครบกำหนดติดตาม', '42', 'คน', CheckCircle2, '#10b981', '#ecfdf5'],
  ['เกินกำหนดติดตาม', '11', 'คน', TimerReset, '#ef4444', '#fff7ed'],
  ['สงสัย SSI', '2', '', AlertTriangle, '#f97316', '#fff7ed'],
  ['ยืนยัน SSI', '1', '', ShieldCheck, '#ef4444', '#fee2e2'],
]

function OpdQueuePage({ patients, loading, search, setSearch }) {
  const [selectedPatient, setSelectedPatient] = useState(null)
  return <div className="space-y-4">
    <section className="grid grid-cols-5 gap-4">{opdMetrics.map(([label, value, unit, Icon, color, bg], index) => <article key={label} className={`flex h-[140px] min-w-0 flex-col rounded-xl border border-black/10 bg-white p-[17px] shadow-sm ${index > 2 ? 'bg-orange-50/40' : ''}`}><div className="flex items-start justify-between gap-2"><p className="whitespace-nowrap text-[14px] font-medium" style={{ color }}>{label}</p><span className="grid size-9 shrink-0 place-items-center rounded-lg" style={{ backgroundColor: bg, color }}><Icon size={20} /></span></div><p className="mt-3 text-[28px] font-semibold leading-8" style={{ color }}>{value} <span className="text-[14px]">{unit}</span></p><p className={`mt-auto whitespace-nowrap text-[12px] leading-4 ${index === 2 ? 'text-red-500' : 'text-[#64748b]'}`}>{index === 0 ? 'ต้องรับเคสวันนี้' : index === 1 ? 'ต้องติดตามวันนี้' : index === 2 ? 'เกินกำหนดแล้ว' : index === 3 ? '↑ 2 ราย จากเมื่อวาน' : '↑ 1 ราย จากเมื่อวาน'}</p></article>)}</section>
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-black/10 bg-white shadow-sm">
      <header className="flex h-[72px] items-center justify-between px-6">
        <h2 className="text-[16px] font-semibold text-[#002d73]">รายการติดตามวันนี้ (48 ราย)</h2>
        <button className="inline-flex h-10 w-[146px] items-center justify-between rounded-lg border border-slate-200 px-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1100px] text-[14px]">
          <thead className="h-10 bg-slate-50">
            <tr>
              {['', 'HN', 'ชื่อผู้ป่วย', 'หัตถการ', 'ศัลยแพทย์', 'ความเสี่ยง SSI', 'สถานะประเมิน SSI', 'เริ่มนัดครั้งแรก'].map((x, i) => (
                <th key={`${x}-${i}`} className="px-4 text-left">
                  {i === 0 ? <input type="checkbox" /> : x}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {!loading &&
              rows.map((p, i) => (
                <tr key={`${p.id}-${i}`} className="h-[70px] border-t border-slate-100">
                  <td className="px-4">
                    <input type="checkbox" />
                  </td>
                  <td className="px-4 font-semibold">{p.id}</td>
                  <td className="px-4">
                    <span
                    ...
                    <small className="block text-slate-500">{p.age} ปี (17 ม.ค. 2501)</small>
                  </td>
                  <td className="px-4">TKA</td>
                  <td className="px-4">นพ กมลชนก อัศวรุ่งโรจน์</td>
                  <td className={`px-4 ${i ? 'text-red-500' : 'text-orange-500'}`}>● {i ? 'สูง' : 'ปานกลาง'}</td>
                  <td className="px-4">
                    <span
                      className={`rounded-lg px-2 py-1 ${i === 0
                    ...
                    >
                      {i === 0 ? 'รอประเมิน' : i === 5 ? 'สงสัย SSI' : i === 6 ? 'ติดเชื้อ SSI' : 'ไม่ติดเชื้อ SSI'}
                    </span>
                  </td>
                  <td className="px-4">15 มิ.ย 2569</td>
                </tr>
              ))}
          </tbody>
        </table>
      </div>
      <footer className="flex h-[46px] items-center justify-between border-t border-slate-200 px-6 text-[12px] text-slate-500">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
      <section className="relative w-full max-w-[480px] rounded-2xl bg-white px-8 pb-7 pt-8 shadow-2xl">
        ...
        <div className="flex justify-center">
          <div className="grid h-[76px] w-[76px] place-items-center rounded-full border-[8px] border-orange-100 bg-orange-50">
            <AlertCircle size={30} className="text-orange-400" strokeWidth={2} />
          </div>
        </div>
